        ...
        '$q', 'savedOrganizationsService', 'cacheService', function ($q, savedOrganizationsService, cacheService) {
            ...
            service.getMyManagedRoles = function() {
                if (common.isTeliaAdmin) {
                    return service.getMySavedOrganizations()
                        .then(function (savedOrganizations) {
                            // Add more organization information, i.e. name
            ...
            service.getUncommonPuis = function() {
                ...
                return service.getAllPuis().then(function(allPuis) {
                    return $.grep(allPuis, function(pui) {
                        return !pui.common;
                    });
                });
            };
            ...
            service.deleteProfile = function(status) {
                ...
                // status is the first argument, remove it
                args.splice(0, 1);
                // add the callback
            ...
            var transformUsers = function(users, isRequest, isInvite) {
                return $.map(users, function (_user) {
                    ...
                    if (isRequest) {
                        // Some API property names differ for users and requests, make them the same.
            ...
            service.getUsersAndRequestsForOrganization = function(organizationTscid) {
                ...
                return $q.all([users, requests]).then(function(data) {
                    var totalUsers = data[0].concat(data[1]);
                    return totalUsers;
                });
            };
            ...
            service.getOrganizations = function(orgs) {
                console.log("SERVICE: getOrganisations", orgs);
                var deferred = $q.defer();
                serviceApi.getOrganizations(orgs, getDataCallback(deferred));
                return deferred.promise;
            };

            service.organizationSearch = function(text) {
                console.log("SERVICE: searchOrganisations:", text);
                var deferred = $q.defer();
                serviceApi.organizationSearch(text, getDataCallback(deferred));
                return deferred.promise;
            };

            service.getOrganizationsInGroup = function() {
                console.log("SERVICE: getOrganizationsInGroup");
                var deferred = $q.defer();
                serviceApi.getOrganizationsInGroup(getDataCallback(deferred));
                return deferred.promise.then(function(data) {
                    return _.map(data, function(item) {
                        return {
                            id: item.organizationNumber,
                            label: item.name + " (" + item.organizationNumber.replace(/([^\-])(\d{4})$/, "$1-$2") + ")"
                        };
                    });
                });
            };

            service.createMasterGroup = function(organisationNumbers, name, phone, website, cost, afNumber, contactPerson, profitabilitySegment, selectedPUIs) {
                console.log('SERVICE: createMasterGroup', organisationNumbers, name, phone, website, cost, afNumber, contactPerson, profitabilitySegment, selectedPUIs);
                var deferred = $q.defer();
                serviceApi.createMasterGroup(organisationNumbers, name, phone, website, cost, afNumber, contactPerson, profitabilitySegment, selectedPUIs, getDataCallback(deferred));
                return deferred.promise;
            };
