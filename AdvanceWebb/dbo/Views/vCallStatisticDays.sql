...
CREATE VIEW dbo.vCallStatisticDays
AS
SELECT DISTINCT 
                      ...
                      SUM(TotalCalls * BlockedCalls / 100)) / CONVERT(REAL, SUM(TotalCalls)), 2) AS BlockedCalls, AdvanceExtId
FROM         dbo.CallStatistic WITH (NOLOCK)
GROUP BY AdvanceExtId, LEFT(CONVERT(varchar(10), CallDateTime, 120), 10)
